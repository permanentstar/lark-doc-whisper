...
def test_default_extensions_config_registers_github_all_readonly_mcp():
    ...
    assert github["url"] == "https://api.githubcopilot.com/mcp/x/all/readonly"
    assert github["headers"]["Authorization"] == "$GITHUB_MCP_AUTHORIZATION"
    assert github["headers"]["X-MCP-Readonly"] == "true"
    assert "X-MCP-Toolsets" not in github["headers"]
