...
3. 在「凭证与基础信息」页复制 **App ID** 与 **App Secret**——分别对应 §3.2 必填项的 `LARK_APP_ID`（#1）与 `LARK_APP_SECRET`（#2），填入 `<repo>/configs/.env`。
...
> ⚠️ **生产环境把三项密钥放在 `<repo>/configs/.env`**，避免当前服务依赖共享的用户级 `~/.env`。
...
| # | 项 | 位置 | 填什么 | 不配的后果 |
|---|---|---|---|---|
| 1 | `LARK_APP_ID` | `configs/.env` | 飞书应用 ID | 启动 `RuntimeError` fail-fast |
...
> 密钥（#1–#3）从 `.env` 加载，候选顺序 `configs/.env` → `~/.env`（取第一个存在的文件；已 export 的同名环境变量优先且不被覆盖）。生产环境把三项放同一个 `configs/.env`。
...
```bash
grep -rn "__fill_me__" configs/ 2>/dev/null   # 应无输出（注释除外）
```
...
# 2. 配置密钥（见 §3）——生产环境使用 repo 内的 configs/.env
cp .env.example configs/.env
# 然后编辑 configs/.env 填入三个密钥
